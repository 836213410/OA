 ...
 */
 @Service
public class DltDepartmentServiceImpl implements DltDepartmentService{
	
	@Autowired
	private DepartmentMapper departmentMapper; // Mapper 
	private static Logger logger = LoggerFactory.getLogger(DltDepartmentServiceImpl.class); // 记录日志
   
    @Override
    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=RuntimeException.class)
    public BaseRes dltLgcDepartment(DltBaseReq req){
	    return oprtDepartment(req, 1); // 操作类型为逻辑删除
	}

	@Override
    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=RuntimeException.class)
    public BaseRes dltRvtDepartment(DltBaseReq req){
        return oprtDepartment(req, 2); // 操作类型为恢复删除
	}

	@Override
    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=RuntimeException.class)
    public BaseRes dltPscDepartment(DltBaseReq req){
        return oprtDepartment(req, 0); // 操作类型为物理删除
	}
	 ...
	 * @param oprtType
     */
    private BaseRes oprtDepartment(DltBaseReq req, int oprtType){
		...
		// 检查要删除的对象是否存在
		DepartmentPo dbDepartmentPo = departmentMapper.findObjByPk(req.getId());
		if(null == dbDepartmentPo) {
		...
		// 物理删除
		if (0 == oprtType) {
			departmentMapper.deleteByPk(req.getId());
			res.setResMsg("物理删除成功");
		}
		// 逻辑删除
		else if (1 == oprtType) {
			dbDepartmentPo.setDltFlg((short)1);
			departmentMapper.updateByPk(dbDepartmentPo);
			res.setResMsg("逻辑删除成功");
		}
		// 恢复删除
		else {
			dbDepartmentPo.setDltFlg((short)0);
			departmentMapper.updateByPk(dbDepartmentPo);
